...
if __name__ == '__main__':
    ...
    def do_eval(dmv_model, m_model, pos, options, epoch):
        ...
        if not options.load_model:
            ml_sentences = utils.read_ml_corpus(options.language_path, options.dev, stc_length=15, isPredict=True, isadd=False)
        else:
            ml_sentences = utils.read_ml_corpus(options.language_path, options.dev, stc_length=15, isPredict=True, isadd=True)
    ...
    if False:# (not options.load_model) or (not os.path.exists(load_file)):
    ...
    if False:#(not options.load_model) or (not os.path.exists(loaded_file)):
