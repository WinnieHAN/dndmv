...
trains = np.array(['eu', 'nl', 'fr', 'de', 'it', 'pl', 'pt', 'es', 'eu', 'nl', 'fr', 'de', 'it', 'pl', 'pt', 'es'])
devs = trains
...
def main():
    ...
    print(end)
    ml_comb_type = sys.argv[3]
    stc_model_type = sys.argv[4]
    for i in range(st, end):
        #trains = 'et-la_ittb-no-fi-grc-nl-en-de-ja-bg-it-hi-fr-eu-sl'#near_langs_list[i]
        #dev = langs[i]
        pcmd = "python src/ml_dmv_parser.py " + ' --sample_idx ' + str(idx[i]) + ' --child_neural --em_type em --cvalency 2 --do_eval --ml_comb_type '+ str(ml_comb_type)  +' --stc_model_type '+ str(stc_model_type)+ ' --em_iter 1  --non_neural_iter 0 --non_dscrm_iter 60 --epochs 90 --function_mask --train '\
               + trains[i] + ' --dev ' + devs[i] + " -idx " + str(idx[i])
        print(pcmd)
        arglist.append(pcmd)
